...
class PersonalAgent:
    ...
    def set_initial_emotion(self):
        time_of_day = self.get_time_of_day()
        if time_of_day == "Morning":
            self.current_emotion = "energitic"
        elif time_of_day == "Afternoon":
            self.current_emotion = "calm"
        elif time_of_day == "Evening":
            self.current_emotion = "relaxed"
        elif time_of_day == "Night Time":
            self.current_emotion = "sleepy"
        else:
            self.current_emotion = random.choice(self.emotions)

    def load_past_emotion(self):
        if not self.memory.empty:
            self.current_emotion = self.memory["Emotion"].iloc[-1]
